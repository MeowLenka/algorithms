...
int main()
{
     ...
     * конструктора. -- для класса Rect не существует конструктора по умолчанию
     *
     ...
     *
     * Раскоментируйте блок кода ниже. 
     * Какие конструкторы вызываются в коде этого блока? -- автоматический конструктор копирования 
     * Кто определил эти конструкторы? -- компиллятор
     * Что они делают в этом случае и в общем случае? -- побитовое копирование (this->x1 = other.x1; ), ??
     */
     ...
     *
     * Что такое деструктор? -- функция, которая выполняется при уничтожении объекта класса
     * Когда он вызывается? -- когда объект выходит из области или явно уничтожается
     * Есть ли деструктор в вашем классе `Rect`? -- да
     * Что делает такой деструктор в вашем случае и в общем случае? --
     * автоматический деструктор - вызывает деструкторы встроенных объектов данного
     * класса; - вызывает деструкторы базовых классов.
     *
     ...
     * вызывается деструктор. -- после выхода из {}
     */
    ...
    // {
    //     Rect r1;
    //     std::cout << r1.left << std::endl;  -- left is a private member of Rect
    // }
     ...
     * 
     * ф-ч объявлена в области видимости своего класса, ее имя не видно за пределами этой области
     * к ф-ч можно обратиться с помощью одного из операторов доступа к членам – точки (.) или стрелки (->) + только через объект класса    
    */
    ...
    if (0){
        Rect r1(1, 2, 3, 4), r2(5, 6, 7, 8); // kons w params 0x16dac2bbc, 0x16dac2bac
        Rect r3 = bounding_rect(r1, r2); // copy kons 0x16dac2b7c 0x16dac2b6c, kons w params 0x16dac2b8c
        // dest 0x16dac2b6c, 0x16dac2b7c
        print_rect(r3);
        // dest 0x16dac2b8c, 0x16dac2bac, 0x16dac2bbc
    }
    ...
    if (0){
        ...
        std::cout << "------внесли изменения------\n";
        r.set_height(20); r.set_width(15);
        print_rect(r);
        std::cout << "ширина: " << r.get_width() << '\n';
        std::cout << "высота: " << r.get_height() << '\n';
        std::cout << "площадь: " << r.get_square() << '\n';
